...
(function () {
    ...
    class DX3rdActorSheet extends ActorSheetClass {
        ...
        async _onAbilityDiamondClick(event) {
            ...
            const ability = $target.data('ability') || $target.closest('[data-ability-id]').data('ability-id');
            if (!ability) return;
            ...
            const isEnemy = this.actor.type === 'enemy';

            // 신드롬 보너스 계산 (에너미는 신드롬/웍스 없음)
            let syndromeBonus = 0;
            let worksBonus = 0;
            if (!isEnemy) {
            ...
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/ability-dialog.html", {
                title,
                abilityLabel,
                point: attrs.point,
                extra: attrs.extra,
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    function updateTotalAndActor() {
                        ...
                        const p = isEnemy ? (Number(this.actor.system.attributes[ability].point) || 0) : (Number($point.val().replace('+', '')) || 0);
                        const s = isEnemy ? 0 : (Number($syndrome.val().replace('+', '')) || 0);
                        const w = isEnemy ? 0 : (Number($works.val().replace('+', '')) || 0);
                        ...
                        if (isEnemy) {
                            this.actor.update({ [`system.attributes.${ability}.extra`]: e });
                        } else {
                            this.actor.update({
                    ...
                    if (!isEnemy) $point.on("input", updateTotalAndActor.bind(this));
                }
            }).render(true);
        }
        ...
        async _onEnemyHPMaxClick(event) {
            ...
            // 에너미 타입만 처리
            if (this.actor.type !== 'enemy') return;
            ...
            const hp = attrs.hp || {};
            const title = game.i18n.localize("DX3rd.HP") + " 수정";
                ...
                if (item.system?.attributes) {
                    for (const [attrKey, attrData] of Object.entries(item.system.attributes)) {
                        if (attrData.key === 'hp' || attrData.key === 'hp_max') {
                            itemBonus += window.DX3rdFormulaEvaluator.evaluate(
                                attrData.value,
                                item,
                                this.actor
                            );
            ...
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/hp-dialog.html", {
                title,
                base: base,
                bonus: totalBonus,
                itemBonus: itemBonus,
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    const $base = html.find("#hp-base");
                    const $bonus = html.find("#hp-bonus");
                    const $total = html.find("#hp-total");
                    
                    function updateTotalAndActor() {
                        const b = Number($base.val()) || 0;
                        const total = b + totalBonus;
                        $total.val(total);
                        
                        this.actor.update({
                            [`system.attributes.hp.base`]: b
                        });
                    }
                    
                    $base.on("input", updateTotalAndActor.bind(this));
                }
            }).render(true);
        }
        ...
        async _onEnemyInitClick(event) {
            ...
            const title = game.i18n.localize("DX3rd.Init") + " 수정";
            ...
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/init-dialog.html", {
                title,
                base: base,
                bonus: totalBonus,
                itemBonus: itemBonus,
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    const $base = html.find("#init-base");
                    const $bonus = html.find("#init-bonus");
                    const $total = html.find("#init-total");
        ...
        async _onEnemyMoveClick(event) {
            ...
            let itemBattleBonus = 0;
            let itemFullBonus = 0;
            for (const item of activeItems) {
                if (item.system?.attributes) {
                    for (const [attrKey, attrData] of Object.entries(item.system.attributes)) {
                        if (attrData.key === 'move' || attrData.key === 'move_battle' || attrData.key === 'battleMove') {
                            itemBattleBonus += window.DX3rdFormulaEvaluator.evaluate(
                                attrData.value,
                                item,
                                this.actor
                            );
                        ...
                        if (attrData.key === 'move_full' || attrData.key === 'fullMove') {
                            itemFullBonus += window.DX3rdFormulaEvaluator.evaluate(
                                attrData.value,
                                item,
                                this.actor
                            );
            ...
            let appliedBattleBonus = 0;
            let appliedFullBonus = 0;
            const appliedEffects = attrs.applied || {};
            for (const [appliedKey, appliedEffect] of Object.entries(appliedEffects)) {
                if (appliedEffect && appliedEffect.attributes) {
                    for (const [attrName, attrValue] of Object.entries(appliedEffect.attributes)) {
                        ...
                        if (aKey === 'move' || aKey === 'move_battle' || aKey === 'battleMove') {
                            appliedBattleBonus += Number(aVal) || 0;
                        }
                        if (aKey === 'move_full' || aKey === 'fullMove') {
                            appliedFullBonus += Number(aVal) || 0;
                        }
                    }
                }
            }
            
            const totalBattleBonus = itemBattleBonus + appliedBattleBonus;
            const totalFullBonus = itemFullBonus + appliedFullBonus;
            const base = move.base || 0;
            const battleTotal = base + totalBattleBonus;
            const fullTotal = battleTotal * 2 + totalFullBonus;
            
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/move-dialog.html", {
                title,
                base: base,
                battleBonus: totalBattleBonus,
                fullBonus: totalFullBonus,
                itemBattleBonus: itemBattleBonus,
                appliedBattleBonus: appliedBattleBonus,
                itemFullBonus: itemFullBonus,
                appliedFullBonus: appliedFullBonus,
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    const $base = html.find("#move-base");
                    const $battleBonus = html.find("#move-battle-bonus");
                    const $fullBonus = html.find("#move-full-bonus");
                    const $battleTotal = html.find("#move-battle-total");
                    const $fullTotal = html.find("#move-full-total");
                    
                    function updateTotalAndActor() {
                        const b = Number($base.val()) || 0;
                        const battleTotal = b + totalBattleBonus;
                        const fullTotal = battleTotal * 2 + totalFullBonus;
                        $battleTotal.val(battleTotal);
                        $fullTotal.val(fullTotal);
                        
                        this.actor.update({
                            [`system.attributes.move.base`]: b
                        });
                    }
        ...
        async _onEnemyEvasionClick(event) {
                ...
                if (appliedEffect && appliedEffect.attributes) {
                    for (const [attrName, attrValue] of Object.entries(appliedEffect.attributes)) {
                        ...
                        if (aKey === 'dodge_add' || aKey === 'dodge_achievement') {
                            appliedAchievementBonus += Number(aVal) || 0;
                        }
                    }
                }
            }
                ...
                if (appliedEffect && appliedEffect.attributes) {
                    for (const [attrName, attrValue] of Object.entries(appliedEffect.attributes)) {
                        ...
                        const aVal = (typeof attrValue === 'object' && 'value' in attrValue) ? attrValue.value : 
                                    (typeof attrValue === 'boolean') ? 0 : window.DX3rdFormulaEvaluator.evaluate(attrValue);
                        if (aKey === 'dodge_dice') {
                            appliedDiceBonus += (Number(aVal) || 0) * 2;
                        }
                    }
                }
            }
            
            const totalAchievementBonus = dodgeAchievementBonus + appliedAchievementBonus;
            const totalDiceBonus = dodgeDiceBonus + appliedDiceBonus;
            const totalBonus = totalAchievementBonus + totalDiceBonus;
            const base = evasion.base || 0;
            const disabled = evasion.disabled || false;
            const total = disabled ? '-' : (base + totalBonus);
            
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/evasion-dialog.html", {
                title,
                base: base,
                bonus: totalBonus,
                achievementBonus: totalAchievementBonus,
                diceBonus: totalDiceBonus,
                itemAchievementBonus: dodgeAchievementBonus,
                appliedAchievementBonus: appliedAchievementBonus,
                itemDiceBonus: dodgeDiceBonus,
                appliedDiceBonus: appliedDiceBonus,
                total: total,
                disabled: disabled
            });
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    const $base = html.find("#evasion-base");
                    const $bonus = html.find("#evasion-bonus");
                    const $total = html.find("#evasion-total");
                    const $disabled = html.find("#evasion-disabled");
                    
                    function updateTotalAndActor() {
                        const isDisabled = $disabled.is(':checked');
                        const b = Number($base.val()) || 0;
                        const total = isDisabled ? '-' : (b + totalBonus);
                        $total.val(total);
                    ...
                    $base.on("input", updateTotalAndActor.bind(this));
                    $disabled.on("change", updateTotalAndActor.bind(this));
                }
            }).render(true);
        }
        ...
        async _onEnemyArmorClick(event) {
            ...
            const total = Math.max(0, base + totalBonus);
            
            const content = await renderTemplate("systems/dx3rd-emanim/templates/dialog/armor-dialog.html", {
                title,
                base: base,
                bonus: totalBonus,
                itemBonus: itemBonus,
            ...
            new Dialog({
                ...
                render: html => {
                    ...
                    const $base = html.find("#armor-base");
                    const $bonus = html.find("#armor-bonus");
                    const $total = html.find("#armor-total");
                    
                    function updateTotalAndActor() {
                        const b = Number($base.val()) || 0;
                        const total = Math.max(0, b + totalBonus);
                        $total.val(total);
                        
                        this.actor.update({
                            [`system.attributes.armor.base`]: b
                        });
                    }
        ...
        async _onActorTypeClick(event) {
            ...
            // 에너미 타입일 때 기본값은 Troop, 캐릭터 타입일 때 기본값은 NPC
            let defaultType = "NPC";
            if (this.actor.type === 'enemy') {
                defaultType = "Troop";
            }
            const currentType = this.actor.system.actorType || defaultType;

            // 템플릿 로드
            const template = "systems/dx3rd-emanim/templates/dialog/actor-type-dialog.html";
