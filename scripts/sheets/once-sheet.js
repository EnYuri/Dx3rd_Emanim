...
class DX3rdOnceSheet extends window.DX3rdItemSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/dx3rd-emanim/templates/item/once-sheet.html",
      width: 520,
      height: 480
    });
  }
  ...
  async getData(options) {
    ...
    if (this.item.system?.used) {
      ...
      data.system.used.max = this.item.system.used.max ?? data.system.quantity;
      ...
      data.system.used.max = data.system.quantity;
  ...
  activateListeners(html) {
    ...
    html.on('change', 'input[name="system.quantity"]', this._onQuantityChange.bind(this));
    ...
    html.on('change', 'input[name^="system."]', async (event) => {
      ...
      try {
        ...
        console.error("DX3rd | OnceSheet input update failed", error);
      }
    });
    ...
    html.on('change', 'select[name^="system."]:not([name$=".key"]):not([name="system.getTarget"]):not([name="system.scene"]):not([name="system.effect.disable"]):not([name="system.effect.runTiming"]):not([name="system.active.disable"]):not([name="system.active.runTiming"]):not([name="system.used.disable"])', async (event) => {
      ...
      try {
        ...
        console.error("DX3rd | OnceSheet select update failed", error);
      }
    });
    ...
    html.on('change', 'textarea[name^="system."]', async (event) => {
      ...
      try {
        ...
        console.error("DX3rd | OnceSheet textarea update failed", error);
      }
    });
  }
  ...
  async _updateObject(event, formData) {
    ...
    // 일회용 아이템의 경우 used.max는 quantity와 연동되므로 제외
    if (formData['system.used.max'] !== undefined) {
      delete formData['system.used.max'];
    }
  ...
  _onQuantityChange(event) {
    const quantity = parseInt(event.currentTarget.value) || 1;
