...
window.DX3rdPsionicHandler = {
    async handle(actorId, itemId, getTarget, options = {}) {
        ...
        if (rollType === '-') {
            ...
            await this.handlePsionicRoll(actor, item, rollType, getTarget, options);
        }
    },
     ...
     */
    async handlePsionicRoll(actor, item, rollType, getTarget, options = {}) {
        ...
        if (!item.system?.weaponSelect && item.system?.attackRoll && item.system.attackRoll !== '-') {
            ...
            if (hasAvailableWeapons) {
                ...
                await this.handlePsionicRollWithWeapon(actor, item, rollType, weaponBonus, options);
                return;
            }
     ...
     * 무기 보너스를 적용한 판정 처리
     */
    async handlePsionicRollWithWeapon(actor, item, rollType, weaponBonus, options = {}) {
        const handler = window.DX3rdUniversalHandler;
